...
- Electron main creates a random per-launch bearer token and supervises the Python service and browser worker. The worker is restarted at most three consecutive times with bounded backoff; a five-second reconnect deadline and service-side 15-second heartbeat lease prevent an unresponsive child from appearing healthy indefinitely.
...
- Every browser-worker process announces a unique session ID. On connection, the workflow coordinator classifies durable runs before sending work: it reapplies pause gates, checkpoints and replays only navigation or app-owned synthetic work, pauses states adjacent to account or submission side effects, and marks an interrupted submission uncertain. Local WebSocket dispatch may retry once with the same command ID; the worker keeps a bounded result cache so a duplicate command returns its original response without executing twice. Commands containing field values remain memory-only and are never written to a recovery checkpoint.
