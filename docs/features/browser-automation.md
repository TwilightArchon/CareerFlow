...
Each worker process has a unique session ID and emits five-second heartbeats. A missing heartbeat expires after 15 seconds, closes the local socket, and lets Electron restart the worker at most three consecutive times with backoff. Reconnection invokes durable workflow recovery before accepting new work. Local command dispatch retries once with the same command ID; an in-flight guard and bounded 500-result cache prevent duplicate execution within a worker process. Recovery regenerates safe commands from durable state and never persists field-value payloads.

The normal suite verifies fixture isolation, sensitivity classification, contracts, mapping, policy, control gates, checkpoints, recovery classification, restart budgets, idempotency, and workflow history. `pnpm browser:test:integration` opens real visible Google Chrome and verifies scan, fill, state change, and stale-plan rejection. A packaged 0.1.11 crash-injection smoke test terminated only the verified worker child, observed a replacement process and authenticated WebSocket connection, and kept the Electron app and Python service running. Real ATS inspection/filling, uploads, agent-service restart, durable action-result deadlines, and confirmation capture remain pending.
